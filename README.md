...
docker run -d --name 1panel --restart unless-stopped \
  ...
  caijiamx/1panel:dood-2.1.7-ubuntu
```
...
# Alpine
docker pull caijiamx/1panel:dood-2.1.7-alpine
```

Replace {version} with 2.0.0~2.1.7 and {os} with ubuntu/centos/alpine.
...
make build OS=ubuntu VERSION=2.1.7 ONEPANEL_TYPE=pro
```
...
1. Build/pull the new image (e.g., 2.1.7)
...
sqlite3 /opt/1panel/db/agent.db "UPDATE settings SET value='v2.1.7' WHERE key='SystemVersion';"
```
...
7. Version v2.1.7 improves the Docker service decision logic, and the Panel->Container function is basically available (the full functionality has not been fully tested).
8. Version v2.1.7 adds disk management, which is not recommended.
